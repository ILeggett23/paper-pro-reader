# Install Paper Pro Reader 0.6.0-rc3
...
pending; keep the rollback copy until RC3 live ink is physically qualified.
...
Do not install Toltec or the old rm2fb/systemd launcher for Paper Pro RC3.
...
```sh
shasum -a 256 -c paper-pro-reader-remarkable-aarch64-0.6.0-rc3-*.tar.xz.sha256
```
...
```sh
ssh root@<PAPER_PRO_IP> 'cd /home/root/xovi/exthome/appload && tar -cJf /home/root/paper-pro-reader-before-rc3.tar.xz koreader'
scp root@<PAPER_PRO_IP>:/home/root/paper-pro-reader-before-rc3.tar.xz .
```
...
```sh
scp paper-pro-reader-remarkable-aarch64-0.6.0-rc3-*.tar.xz root@<PAPER_PRO_IP>:/home/root/
ssh root@<PAPER_PRO_IP>
cd /home/root
tar -xJf paper-pro-reader-remarkable-aarch64-0.6.0-rc3-*.tar.xz
...
RC2 directory before changing it:

```sh
ssh root@<PAPER_PRO_IP> 'cd /home/root/xovi/exthome/appload && tar -cJf /home/root/paper-pro-reader-rc2-before-rc3.tar.xz koreader'
scp root@<PAPER_PRO_IP>:/home/root/paper-pro-reader-rc2-before-rc3.tar.xz .
scp paper-pro-reader-remarkable-aarch64-0.6.0-rc3-*.tar.xz root@<PAPER_PRO_IP>:/home/root/
ssh root@<PAPER_PRO_IP>
cd /home/root/xovi/exthome/appload
tar -xJf /home/root/paper-pro-reader-remarkable-aarch64-0.6.0-rc3-*.tar.xz
exit
```
...
Do not rebuild Xovi merely for this application upgrade; the first RC3 retest
...
that extension. If the error recurs or AppLoad cannot launch RC3, record its
...
stack before the first RC3 live-ink test when RC3 launches normally.
...
ssh root@<PAPER_PRO_IP>
cd /home/root/xovi/exthome/appload
mv koreader koreader-rc3-failed
cd /home/root
tar -xJf paper-pro-reader-rc2-before-rc3.tar.xz -C /home/root/xovi/exthome/appload
exit
```

Keep `koreader-rc3-failed` until the restored build launches and data is
