# Install Paper Pro Reader 0.6.0-rc2
...
pending; keep the rollback copy until RC2 touch input is physically qualified.
...
Do not install Toltec or the old rm2fb/systemd launcher for Paper Pro RC2.
...
```sh
shasum -a 256 -c paper-pro-reader-remarkable-aarch64-0.6.0-rc2-*.tar.xz.sha256
```
...
```sh
ssh root@<PAPER_PRO_IP> 'cd /home/root/xovi/exthome/appload && tar -cJf /home/root/paper-pro-reader-before-rc2.tar.xz koreader'
scp root@<PAPER_PRO_IP>:/home/root/paper-pro-reader-before-rc2.tar.xz .
```
...
```sh
scp paper-pro-reader-remarkable-aarch64-0.6.0-rc2-*.tar.xz root@<PAPER_PRO_IP>:/home/root/
ssh root@<PAPER_PRO_IP>
cd /home/root
tar -xJf paper-pro-reader-remarkable-aarch64-0.6.0-rc2-*.tar.xz
...
RC1 directory before changing it:

```sh
ssh root@<PAPER_PRO_IP> 'cd /home/root/xovi/exthome/appload && tar -cJf /home/root/paper-pro-reader-rc1-before-rc2.tar.xz koreader'
scp root@<PAPER_PRO_IP>:/home/root/paper-pro-reader-rc1-before-rc2.tar.xz .
scp paper-pro-reader-remarkable-aarch64-0.6.0-rc2-*.tar.xz root@<PAPER_PRO_IP>:/home/root/
ssh root@<PAPER_PRO_IP>
cd /home/root/xovi/exthome/appload
tar -xJf /home/root/paper-pro-reader-remarkable-aarch64-0.6.0-rc2-*.tar.xz
exit
```
...
Do not rebuild Xovi merely for this application upgrade; the first RC2 retest
...
that extension. If the error recurs or AppLoad cannot launch RC2, record its
...
stack before the first RC2 tap test when RC2 launches normally.
...
ssh root@<PAPER_PRO_IP>
cd /home/root/xovi/exthome/appload
mv koreader koreader-rc2-failed
cd /home/root
tar -xJf paper-pro-reader-rc1-before-rc2.tar.xz -C /home/root/xovi/exthome/appload
exit
```

Keep `koreader-rc2-failed` until the restored build launches and data is
